...
@section('content')
    ...
    <form method="POST" action="/shortcuts/department/{{$shortcut->id}}/edit">
        {{ csrf_field() }}
        ...
        <div class="department_list">
            @php $departments = $shortcut->departments()->get()->pluck('id')->toArray(); @endphp
            @foreach(Auth::user()->departments()->get() as $department)
                <div class="form-group">
                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="department[]" id="" value="{{$department->id}}" @if(in_array($department->id, $departments))checked @endif >
                            {{$department->name}}
                        </label>
                    </div>
                </div>
